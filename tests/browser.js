...
document.querySelector('#run').onclick=async()=>{
  ...
  try{
    ...
    click('[data-nav="dashboard"]');click('[data-mode="race"]');set('#race-screen','side');set('#race-pitLap',0);
    assert(saved().race.splitScreen==='side','Split layout saves from Grand Prix setup');
    click('[data-action="start-race"]');await sleep(300);
    assert(doc().querySelectorAll('.split-pane').length===2,'Split screen creates two player views');
    ...
    assert(doc().querySelector('#split-pit-1').textContent==='PIT REQUESTED'&&doc().querySelector('#split-pit-0').textContent==='NO PLANNED STOP','P requests a pit stop for P2 only');
    press('keydown','b');press('keyup','b');await sleep(150);assert(doc().querySelector('#split-pit-0').textContent==='PIT REQUESTED','B requests P1 pit stop');
    press('keydown','Backspace');press('keyup','Backspace');await sleep(150);
    assert(doc().querySelector('#split-timing-1').textContent.includes('+5s')&&doc().querySelector('#split-timing-0').textContent.includes('+0s'),'P2 recovery penalty is independent');
    click('[data-action="pause"]');
    assert(!doc().querySelector('[data-action="request-pit"]')&&doc().querySelector('#split-tire-1'),'Split pause has separate strategies and no pit request button');
    const times=[0,1].map(id=>doc().querySelector(`#split-timing-${id}`).textContent);await sleep(200);
    assert([0,1].every(id=>doc().querySelector(`#split-timing-${id}`).textContent===times[id]),'Pause freezes both players');
    set('#split-tire-0','hard');set('#split-tire-1','wet');set('#split-fuel-1',44);set('#split-layout','stacked');
    bounds=panes();assert(bounds[1].top>bounds[0].top&&Math.abs(bounds[1].left-bounds[0].left)<2,'Top/bottom layout places players above and below');
    click('[data-action="pause"]');press('keydown','Backspace');press('keyup','Backspace');await sleep(150);
    assert(doc().querySelector('#split-status-1').textContent.includes('44.0 kg · wet')&&!doc().querySelector('#split-status-0').textContent.includes('wet'),'P2 service settings do not change P1');
    assert([0,1].every(id=>doc().querySelector(`#split-canvas-${id}`).dataset.drawError==='0'),'Both cameras resize cleanly after layout change');
    click('[data-action="pause"]');click('[data-action="end-session"]');
    assert(doc().querySelector('#race-overlay').textContent.includes('Player 2'),'Split results include both players');
    click('[data-action="race-again"]');await sleep(250);assert(doc().querySelector('#split-screen').classList.contains('stacked'),'Replay retains selected split layout');
    click('[data-action="pause"]');click('[data-action="end-session"]');click('[data-nav="dashboard"]');click('[data-mode="race"]');set('#race-screen','solo');click('[data-action="start-race"]');await sleep(200);click('[data-action="pause"]');
    assert(doc().querySelector('#race-canvas')&&doc().querySelector('[data-action="request-pit"]'),'Returning to solo restores its camera and pit-request pause button');
    click('[data-action="end-session"]');
